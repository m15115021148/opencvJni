...
if(!initialized_already)
{
		...
		for (int i = 0; i < num_images; ++i)
		{
			...
			if (full_img.empty())
			{
				//LOGE("Can't open image %s", img_names[i]);
				return empty;
			}
			if (work_megapix < 0)
			{
		...
		LOGD("Pairwise matching, time: %f sec" ,((getTickCount() - t) / getTickFrequency()));
		...
		LOGD("Warping images, time: %f sec" ,((getTickCount() - t) / getTickFrequency()));
